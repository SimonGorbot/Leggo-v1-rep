...
//long motor1_time[num_points] = {0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000, 16000};
...
//  Serial.print(time2A); Serial.print(" | "); Serial.print(angle2A); Serial.print(" | "); Serial.println(slope2);
  ...
  while ((time1B - timeCurrent) >= 0){
    ...
    if (steps > 0){
      ...
      //timePrevious1 = (steps * motorAcc) / slope1;
      Serial.print("ONE\t"); Serial.print(steps); Serial.print(" | "); Serial.println(stepsDir);
    }
    m2_calculate_angle(slope2, timeCurrent, timePrevious2);
    if (steps > 0){
      m2_move_motor(stepsDir, steps);
      timePrevious2 = timeCurrent;
      //timePrevious2 = (steps * motorAcc) / slope2;
      Serial.print("TWO\t"); Serial.print(steps); Serial.print(" | "); Serial.println(stepsDir);
    }
  }
